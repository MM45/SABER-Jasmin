...
void indcpa_kem_keypair_randominc(unsigned char *pk, unsigned char *sk, unsigned char seed[SABER_SEEDBYTES], unsigned char noiseseed[SABER_COINBYTES]); // TODO: indcpa with randomness included instead of via randombytes
...
void InnerProd(uint16_t pkcl[SABER_K][SABER_N],uint16_t skpv[SABER_K][SABER_N],uint16_t mod,uint16_t res[SABER_N]); // TODO: Only in header for unit test purposes

void MatrixVectorMul(polyvec *a, uint16_t skpv[SABER_K][SABER_N], uint16_t res[SABER_K][SABER_N], uint16_t mod, int16_t transpose); // TODO: Only in header for unit test purposes

void POL2MSG(uint16_t *message_dec_unpacked, unsigned char *message_dec); // TODO: Only in header for unit test purposes

void GenMatrix(polyvec *a, const unsigned char *seed); // TODO: Only in header for unit test purposes
...
void indcpa_kem_keypair_randominc_jazz(unsigned char *pk, unsigned char *sk, unsigned char seed[SABER_SEEDBYTES], unsigned char noiseseed[SABER_COINBYTES]); // TODO: indcpa with randomness included instead of via randombytes
