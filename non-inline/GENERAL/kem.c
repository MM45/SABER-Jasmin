...
int crypto_kem_keypair_randominc(unsigned char *pk, unsigned char *sk, unsigned char random_bytes_crypto[SABER_KEYBYTES], unsigned char indcpa_seed[SABER_SEEDBYTES], unsigned char indcpa_noiseseed[SABER_COINBYTES])
{
  ...
  indcpa_kem_keypair_randominc(pk, sk, indcpa_seed, indcpa_noiseseed);               // sk[0:SABER_INDCPA_SECRETKEYBYTES-1] <-- sk
  ...
  for (i = 0; i < SABER_KEYBYTES; ++i) {
      sk[SABER_SECRETKEYBYTES - SABER_KEYBYTES + i] = random_bytes_crypto[i];
  }
...
/*
  for (i = 0; i < 32; ++i) {
      k[i] = kr[32 + i];
      k[32 + i] = *(sk+SABER_SECRETKEYBYTES-SABER_KEYBYTES + i);
  }
  return 0;
  */
  cmov(kr, sk+SABER_SECRETKEYBYTES-SABER_KEYBYTES, SABER_KEYBYTES, fail); 
