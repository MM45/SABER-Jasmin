...
int main() 
{
	...
	unsigned char h0_c[SHAKE128_RATE];
	unsigned char h0_jazz[SHAKE128_RATE];
	unsigned char h1_c[SHAKE128_RATE];
	unsigned char h1_jazz[SHAKE128_RATE];
	unsigned char h2_c[SHAKE128_RATE];
	unsigned char h2_jazz[SHAKE128_RATE];
	unsigned char h3_c[SHAKE128_RATE];
	unsigned char h3_jazz[SHAKE128_RATE];
	uint64_t temp[100];
	__m256i s_c[25];
	__m256i s_jazz[25];
	uint64_t* s64_c;
	...
	random_test_bytes(h0_c, SHAKE128_RATE);
	random_test_bytes(h1_c, SHAKE128_RATE);
	random_test_bytes(h2_c, SHAKE128_RATE);
	random_test_bytes(h3_c, SHAKE128_RATE);
	...
	// Test
	keccak_squeezeblocks4x(h0_c, h1_c, h2_c, h3_c, 1, s_c, SHAKE128_RATE);
	keccak_squeezeblocks4x_128_128_jazz(h0_jazz, h1_jazz, h2_jazz, h3_jazz, s_jazz);
	
	for (i = 0; i < SHAKE128_RATE; ++i) {
		if (h0_c[i] != h0_jazz[i]) {
			printf("[!] Unit test failed.\nFunction:\tkeccak_squeezeblocks4x_128_128.\nReason:\th0_c[%d] != h0_jazz[%d] ==> %d != %d\n", i, i, h0_c[i], h0_jazz[i]);
		}
	}
