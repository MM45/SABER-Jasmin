/*** test_kem_regular_crypto_kem_debug.c: File containing the unit test for the crypto_kem functions, with (regular) SABER parameters, for debugging ***/
...
int main()
{
	...
	//random_test_bytes(sk_c, SABER_SECRETKEYBYTES);
	crypto_kem_dec_debug(k_b_c, c_c, sk_c);
...
/*	for (i = 0; i < SABER_SECRETKEYBYTES; ++i) {
	...
	crypto_kem_dec_debug_jazz(k_b_jazz, c_jazz, sk_jazz);
...
/*	for (i = 0; i < SABER_SECRETKEYBYTES; ++i) {
		printf("sk_c[%d] -- sk_jazz[%d] ==> %d -- %d\n", i, i, sk_c[i], sk_jazz[i]);

	}

	for (i = 0; i < SABER_BYTES_CCA_DEC; ++i) {
			printf("c_c[%d] -- c_jazz[%d] ==> %d -- %d\n", i, i, c_c[i], c_jazz[i]);
	}*/
