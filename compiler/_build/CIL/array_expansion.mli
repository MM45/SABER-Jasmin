 ...
 sig
  module M :
   sig
    ...
    val get : expansion -> Var.var -> coq_Z -> (wsize * Equality.sort) option
    ...
    val remove_alloc : Equality.sort -> alloc_t -> array_info Mvar.Map.t
  ...
  val check_lval :
    (stype * pexpr) option -> lval -> lval -> M.expansion -> M.expansion cexec
 end
...
module CheckExpansion :
 sig
  val loop :
    instr_info -> (CBEAU.M.t -> CBEAU.M.t ciexec) -> nat -> CBEAU.M.t ->
    CBEAU.M.t ciexec

  val loop2 :
    instr_info -> (CBEAU.M.t -> (CBEAU.M.t * CBEAU.M.t) ciexec) -> nat ->
    CBEAU.M.t -> CBEAU.M.t ciexec
  ...
  val check_es :
    pexpr list -> pexpr list -> CBEAU.M.t -> (error_msg, CBEAU.M.t) result

  val check_lvals :
    lval list -> lval list -> CBEAU.M.t -> (error_msg, CBEAU.M.t) result

  val check_var : var_i -> var_i -> CBEAU.M.t -> CBEAU.M.t cexec

  val check_vars :
    var_i list -> var_i list -> CBEAU.M.t -> (error_msg, CBEAU.M.t) result

  val check_i :
    instr_info -> instr_r -> instr_r -> CBEAU.M.t -> CBEAU.M.t ciexec

  val check_I :
    instr -> instr -> CBEAU.M.t -> (instr_info * error_msg, CBEAU.M.t) result

  val check_cmd :
    instr_info -> instr list -> instr list -> CBEAU.M.t ->
    (instr_info * error_msg, CBEAU.M.t) result
